...
Freenet is maturing it is time to re-examine the core Freenet architecture to
see where it can be improved. This new design is known as "Next Generation
Routing".

### Freenet's current routing mechanism

Firstly, let me recap on how Freenet works right now. Every node in the
Freenet network is required to provide a simple service to other nodes in the
system. When a node receives a request for a particular key, it should do its
best to retrieve the data corresponding to that key as quickly as possible
and send it back to the node that requested it. Provided that every node in
the Freenet network performs this service reasonably well - Freenet will work.
...
the network which it thinks will be better able to retrieve the data. The way
Freenet makes this decision forms the core of the Freenet algorithm.

At present, the algorithm used to select which node should be consulted for a
given key is relatively simple. In short, when a node forwards a request for
a particular key to another node in the network, and that node is successful
in retrieving the data, the address of an upstream node (possibly the one
where the data originated) is included in the reply, this is called the
"DataSource". The requester makes a note of the requested key, and the
...
George successfully answered a question about France, he might also be a good
person to answer a question about Belgium.
...
the following image represents the keys stored by an actual Freenet node. The
X axis represents the "keyspace", at the left is key 0, across to the right
which is key 2<sup>160</sup>. Dark areas indicate where the node has better
knowledge. The dark strips indicate areas in which the node has detailed
knowledge about where requests for those keys should be routed:
...
current routing algorithm is performing correctly. Nodes specialize like this
due to an emergent feedback effect, when a node successfully responds to a
request for a given key - it increases the likelihood that other nodes will
route requests for similar keys in the future. Over time this effect causes
the specialization that can be seen very clearly in the diagram above. You
can see an animation of a node's datastore specializing over time [here](
assets/img/histanim.mpeg).
...
dramatically improve a node's ability to route requests in a manner likely to
result in the fastest response for that request. By avoiding guesswork,
and striving for a firm basis in statistical reality - we can even accelerate
...
information including response times for requesting particular keys,
the proportion of requests which succeed in finding information, and the time
...
achieve this an algorithm was required that would meet the following criteria:

* Must be able to make reasonable guesses about keys that it has not seen before

* Must be progressive – if a node's performance changes over time, this should
  be represented, but should not be oversensitive to recent fluctuations which
  may vary significantly from the average.

* Must be “scale free”
  Consider a naive implementation of this that works by splitting the keyspace
  into a number of sections and maintaining an average for each. Now consider a
  node where most of its incoming requests lie within a very small section of
  the keyspace. Our naïve implementation would be unable to represent variations
  in response time within that small area and would therefore limit that node's
  ability to accurately estimate routing times.
...
When we have a new routing time sample for a particular key – we move the two
points closest to our new sample toward it. The amount they are moved can be
adjusted to change how “forgetful” the estimator is.
...
Also consider the fact that there isn't just one timing measurement that must
be taken into account when we receive a DataReply, there are two. The first
is the time required to get the beginning of the response, the second is the
time required to transfer the data. This raises the question of how we can
combine these two timings into a single value which can be reported to the
estimator algorithm described above.
...
live" field (similar to "time to live" in other protocols), a "DataNotFound"
or DNF message is returned to the requester. This indicates that the data
could not be found within the time limit specified by the requester. There
are two reasons that a DNF can be returned for some data, either the data
exists but wasn't found, or the data didn't exist at all. In the former case,
a DNF would indicate a shortcoming in the routing ability of whichever node
the request was routed to. In the latter case, it would not - but the
difficulty is that for a given DNF - there is no easy way to tell what type
...
looking at how long previous DNFs took for requests sent to this node (taking
into account that this time will be proportional to the HTL of the request
<sup>ie.</sup> a request with a HTL of 10 will visit twice as many nodes as a
request with HTL 5 and will therefore take about twice as long to return a
DNF). We can estimate the latter by looking at the average amount of time it
takes to successfully retrieve some data.

Now, imagine a Freenet node with perfect routing, the only DNFs it returns
would be "legitimate" - since if the data was in the network, it would find
it. The proportion of DNFs this perfect node returned would be the same as
the proportion of DNFs for which the data simply didn't exist in the network.
Now - such a node (we assume) could not exist, however we can approximate it
by looking at the proportion of DNFs for the node with the lowest proportion
of DNFs in our routing table.
...
attempt to establish a connection to them - we will be unable to do-so,
we can account for this possibility by recalling the average proportion of
failed connections for each node, and how long each took - and adding this on
to our estimated routing time for that node.

### Inherited Knowledge

One of the problems observed in the current Freenet is the time required for
a Freenet node to establish sufficient knowledge about the Freenet network to
route efficiently. This is particularly damaging to Freenet's usability as
people's first impressions of software is critical, and the first impressions
of Freenet are generally the worst impressions of Freenet as they are formed
before the Freenet node can route requests effectively.
...
with actual statistical data so that even with a node first starts up,
it will already have the knowledge of an experienced node. It will then go on
...
its own performance to the reply - but clearly this would be open to abuse. A
refinement would be to say that any node passing back a reply which has
...
2007 paper by Oskar Sandberg [Decentralized Search with Random Costs](
http://arxiv.org/abs/0804.0577) determined that the concepts underlying this
paper "were fundamentally sound".
