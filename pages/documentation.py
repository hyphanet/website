...
If you want to install the Freenet software on a computer which lacks a GUI, see the [Headless install](download.html#unix) instructions. If you have mirrored a working Freenet installation to a machine without explicitly installing it, see [Mirrored install](download.html#mirrored). Otherwise, continue reading.
...
This page contains links to freely available content on Freenet. If you would like to add a new link please email it to [us](mailto:ian@locut.us) - remember that we will only link to content that can be legally distributed in the United States of America.
""")))
...
Each file that exists on Freenet has a key associated with it. Freenet 0.7 has various types of keys. Keys are used for everything on freenet, and are a kind of [URI](https://en.wikipedia.org/wiki/Uniform_Resource_Identifier) (e.g. freenet:=KSK@sample.txt).
...
You can download the latest pre-compiled version from [https://downloads.freenetproject.org/alpha/plugins/Freemail/](https://downloads.freenetproject.org/alpha/plugins/Freemail/)
...
The <anything> before the @ will be ignored. It is only there because mail user and transport agents want to see an @ in addresses. Yes, we _do_ know this is an ugly kludge.
...
You can easily make [fetchmail](http://fetchmail.berlios.de/) poll your freenet mail with an entry like this:
...
4.  Type in the name and either the long Freemail address you were given, or your short address if you created one. Check this carefully as an incorrect address here will not only mean people won't be able to reply to you, but your messages will appear as 'Spoofed'. Once you've done this, click Next.
5.  Set the type of incoming server to IMAP and the incoming server name to localhost. Then click Next.
6.  For the Incoming User Name, use the account name you chose earlier ('john' in this example), which may not be the same as your email address. Click Next.
7.  Enter an arbitrary Account Name and click Next and then Finish.
8.  Now we have to change the IMAP port number from the default: On the left panel click on Server Settings under the new account. Change the Port to 3143 from the default of 143.
...
...it probably means you downloaded the Freemail plugin through the Web interface and your node is already running it. On a Linux machine you can check to see if the private SMTP and IMAP ports are actually in use with `netstat -tln`; the port numbers you're looking for in the listing are 3143 (Freenet IMAP) and 3025 (Freenet SMTP).

If you get these messages and these ports are _not_ in use, try shutting down and restarting the node. If the problem persists after that, you have found a bug and should file it with the Freenet developers.
...
To run Frost on Linux from the command line, change to your Freenet install directory, then change to the frost subdirectory. Then make the frost.sh file executable, and run it:
...
Additional information about Frost can be found in the mailing-list or on the official website: [Frost Homepage](http://jtcfrost.sourceforge.net/)
""")))
